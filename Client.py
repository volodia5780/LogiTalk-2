...
class MainWindow(CTk):
    def __init__(self):
        ...
        # 🔹 EMOJI BUTTON
        self.emoji_button = CTkButton(self.bottom_frame, text="😊", width=40, command=self.open_emoji_menu)
        self.emoji_button.pack(side="right", padx=5)
    ...
    def open_emoji_menu(self):
        self.emoji_window = CTkToplevel(self)
        self.emoji_window.geometry("220x300")
        self.emoji_window.title("Емодзі / Стікери")

        emojis = ["😀", "😂", "😍", "😎", "😭", "🔥", "👍", "❤️"]
        stickers = ["(╯°□°）╯︵ ┻━┻", "¯\\_(ツ)_/¯", "(づ｡◕‿‿◕｡)づ"]

        CTkLabel(self.emoji_window, text="Емодзі").pack(pady=5)

        for emoji in emojis:
            btn = CTkButton(self.emoji_window, text=emoji, width=40,
                            command=lambda e=emoji: self.insert_emoji(e))
            btn.pack(padx=5, pady=2)

        CTkLabel(self.emoji_window, text="Стікери").pack(pady=5)

        for sticker in stickers:
            btn = CTkButton(self.emoji_window, text=sticker,
                            command=lambda s=sticker: self.insert_emoji(s))
            btn.pack(padx=5, pady=2)
